...
class ConvolutionalNeuralNetworkModel(nn.Module):
    ...
    # Predictor
    def f(self, x):
        return torch.softmax(self.logits4(self.logits3(self.logits2(self.logits1(x)))), dim=1)

    # Cross Entropy loss
    def loss(self, x, y):
        return nn.functional.cross_entropy(self.logits4(self.logits3(self.logits2(self.logits1(x)))), y.argmax(1))
...
if __name__ == '__main__':
    ...
    mnist_train = torchvision.datasets.FashionMNIST('./data/FashionMNIST', train=True, download=True)
    x_train = mnist_train.data.reshape(-1, 1, 28, 28).float().to(device)  # torch.functional.nn.conv2d argument must include channels (1)
    y_train = torch.zeros((mnist_train.targets.shape[0], 10), device=device)  # Create output tensor
    y_train[torch.arange(mnist_train.targets.shape[0]), mnist_train.targets] = 1  # Populate output

    mnist_test = torchvision.datasets.FashionMNIST('./data/FashionMNIST', train=False, download=True)
    x_test = mnist_test.data.reshape(-1, 1, 28, 28).float().to(device)  # torch.functional.nn.conv2d argument must include channels (1)
    y_test = torch.zeros((mnist_test.targets.shape[0], 10), device=device)  # Create output tensor
